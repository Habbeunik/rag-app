...
function validateProcessRequest(body: unknown): {
  ...
  const { text, filename } = body as Partial<ProcessPDFRequest>;
...
function createVectorDocuments(
  ...
  return chunks.map((chunk, index) => ({
    ...
    metadata: {
      ...
      pageNumber: Math.floor(index / CHUNKS_PER_PAGE_ESTIMATE) + 1,
    },
  }));
}
...
export async function POST(
  request: NextRequest
): Promise<NextResponse<ProcessPDFResponse | ErrorResponse>> {
  try {
    ...
    return createErrorResponse("Failed to process PDF", 500);
  }
}
...
export async function GET(
  ...
  NextResponse<
    SearchResponse | ErrorResponse | { documents: unknown[]; total: number }
  >
> {
